          ...
          {orders.map((o) => (
            <Link key={o.id} to={`/orders/${o.id}`} className="block overflow-hidden rounded-[28px] border border-[#f1e4dc] bg-white p-4 shadow-soft transition hover:-translate-y-0.5 hover:shadow-hover sm:p-5">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                <img src={o.items?.[0]?.image || fallbackImage} alt="Order item" onError={(e) => { if (e.currentTarget.src !== fallbackImage) e.currentTarget.src = fallbackImage }} className="h-24 w-full rounded-[20px] object-cover sm:w-28" />
