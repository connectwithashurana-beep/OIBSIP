...
export default function PizzaBuilder() {
      ...
      <div className="mx-auto max-w-4xl px-4 py-10">
        <div className="rounded-lg bg-red-50 p-6 text-center dark:bg-red-900/20">
          ...
          <p className="mt-2 text-sm text-red-600 dark:text-red-300">
            We couldn't reach the menu server. Check your connection and try again.
          </p>
          <button
            type="button"
            onClick={() => { setError(null); loadOptions() }}
  ...
  // Render a per-step empty message instead of a silent blank grid when a
  // category returns no options (e.g. ingredients not seeded yet).
  const renderEmpty = (label) => (
    <div className="rounded-lg border border-dashed border-gray-300 bg-gray-50 p-8 text-center dark:border-gray-600 dark:bg-gray-800">
      <p className="font-semibold text-gray-800 dark:text-gray-200">No {label} available yet.</p>
      <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
        We couldn't find any {label} in the menu. The team is likely still adding them.
      </p>
      <button
        type="button"
        onClick={() => { setError(null); loadOptions() }}
      ...
      <div className="mb-8 rounded-xl bg-white p-6 shadow-sm dark:bg-gray-800">
        ...
        {step === 3 && (
          <div>
            ...
            {options.vegetables.length === 0 ? (
              renderEmpty('vegetables')
            ) : (
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
