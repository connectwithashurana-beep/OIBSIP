...
class MenuPagination(PageNumberPagination):
    """Allow the client to request a larger page via ?limit= (the menu fetches
    the whole catalog in one request). Capped at 100 to stay reasonable."""
...
class PizzaBaseViewSet(PublicReadMixin, viewsets.ModelViewSet):
    queryset = PizzaBase.objects.all().order_by("name")
...
class SauceViewSet(PublicReadMixin, viewsets.ModelViewSet):
    queryset = Sauce.objects.all().order_by("name")
...
class CheeseViewSet(PublicReadMixin, viewsets.ModelViewSet):
    queryset = Cheese.objects.all().order_by("name")
