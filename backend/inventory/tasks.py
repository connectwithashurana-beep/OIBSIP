...
def restore_stock(item_type, item_id, quantity=1):
    """Atomically add stock back when an order is cancelled.

    Mirrors reduce_stock() using an F() expression so concurrent operations
    can't lose an increment. Returns True if the ingredient was found.
    """
    from django.db.models import F
    ...
    if not model:
        return False
    updated = model.objects.filter(id=item_id).update(
