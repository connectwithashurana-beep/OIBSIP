...
from inventory.tasks import restore_stock
...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if from_email and order.user.email:
            send_mail(
                "Order Confirmation",
    ...
    def cancel(self, request, pk=None):
        ...
        # Restore stock that was deducted when this order was created (COD /
        # WALLET) or when its payment was verified (Razorpay). Razorpay orders
        # that were never paid never had stock deducted, so only restore when a
        # payment is recorded as success (or the order used a non-Razorpay
        # method, which deducted stock at creation time).
        was_stock_deducted = order.payment_method.lower() != "razorpay" or (
            getattr(order, "payment", None) is not None
        ...
        if was_stock_deducted:
            for item in order.items.select_related("base", "sauce", "cheese").prefetch_related("vegetables"):
                if item.base_id:
                    restore_stock("base", item.base_id, item.quantity)
                if item.sauce_id:
                    restore_stock("sauce", item.sauce_id, item.quantity)
                if item.cheese_id:
                    restore_stock("cheese", item.cheese_id, item.quantity)
                for veg in item.vegetables.all():
                    restore_stock("vegetable", veg.id, item.quantity)
