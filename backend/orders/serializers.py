...
class OrderItemSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ["id", "pizza_name", "base_name", "sauce_name", "cheese_name", "vegetable_names",
                  "image", "quantity", "unit_price", "total_price"]

    def get_image(self, obj):
        """Expose the preset pizza's image URL so order lists can render it.

        Mirrors PizzaSerializer.get_image priority: image_url first, then the
        uploaded image (absolute when a request context is available), else None.
        """
        pizza = getattr(obj, "pizza", None)
        if not pizza:
            return None
        ...
        if pizza.image:
            ...
            if request:
                return request.build_absolute_uri(pizza.image.url)
            return pizza.image.url
